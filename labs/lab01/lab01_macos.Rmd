...
This tutorial is for Mac users. For Windows users, first read the [tutorial]() to install Git for Windows (Git Bash), then most of bash commands will work in Git Bash.

# Connect from your Mac to teaching server

1. SSH client on MacOS. 
    - MacOS: open the `Terminal` app.  
    - If necessary, change default shell to Bash by following command in Terminal
    ```{bash, eval=F}
    chsh -s /bin/bash
    ```
...
```{bash, eval=F}
git clone git@github.com:ucla-biostat203b-2021winter/ucla-biostat203b-2021winter.github.io.git
```
...
Open your browser and enter address <http://server.ucla-biostat-203b.com:8787> to use RStudio **on the teaching server**.
...
5. Now try to SSH into the teaching server from your Mac **without password**. 
```{bash, eval=F}
ssh USERNAME@server.ucla-biostat-203b.com
```
